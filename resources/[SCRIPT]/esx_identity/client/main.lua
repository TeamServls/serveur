...
RegisterNUICallback('escape', function(data, cb)
	if hasIdentity then
		...
		TriggerEvent('chat:addMessage', { args = { '^1[IDENTITY]', '^1Vous devez créer votre identité avant de jouer' } })
	end
end)
...
function verifyName(name)
	...
	if count ~= nameLength then
		return 'Votre nom contient des caractères spéciaux invalide.'
	end

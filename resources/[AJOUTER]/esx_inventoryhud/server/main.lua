...
AddEventHandler('esx_inventoryhud:updateKey', function(target, type, itemName)
	...
	if type == "item_key" then -- MEETA GiveKey
		...
		MySQL.Async.execute("UPDATE owned_vehicles SET owner = @newplayer WHERE plate=@plate",
		{
		...
		TriggerClientEvent('b1g_notify:client:Notify', source, { type = 'true', text = 'Vous avez donner les clefs' })

		TriggerClientEvent('b1g_notify:client:Notify', target, { type = 'true', text = 'Vous avez maintenant les clefs' })

...
-- GENERAL --
Config.MenuTitle = 'Menu Personnel' -- change it to you're server name
...
Config.Controls = {
	...
	TPMarker = {keyboard1 = Keys['LEFTSHIFT'], keyboard2 = Keys['1']}
}
		...
		command = function()
			...
			if plyId ~= nil then
				...
				if type(plyId) == 'number' then
					TriggerServerEvent('KorioZ-PersonalMenu:teleserver', GetPlayerServerId(PlayerId()), plyId)
				end
			end
		...
		command = function()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					TriggerServerEvent('KorioZ-PersonalMenu:tmtc', amount)
				end
			end
	...
	},
	{
		...
		command = function()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					TriggerServerEvent('KorioZ-PersonalMenu:salut', amount)
				end
			end
	...
	},
	{
		...
		command = function()
			...
			if amount ~= nil then
				...
				if type(amount) == 'number' then
					TriggerServerEvent('KorioZ-PersonalMenu:sale', amount)
				end
			end
...
--	{
--		name = 'showblips',
--		label = _U('admin_showblips_button'),
--		groups = {'_dev', 'owner', 'superadmin', 'admin', 'mod'},
--		command = function()
--			Player.showblips = not Player.showblips
--			print("showblipsCONFIG")
--
--			--[[if not showname then
--				for k, v in pairs(Player.gamerTags) do
--					RemoveMpGamerTag(v)
	...
	},
	{
		...
		command = function()
			...
			if plyId ~= nil then
				...
				if type(plyId) == 'number' then
					TriggerServerEvent('esx_ambulancejob:revive2', plyId)
				end
			end

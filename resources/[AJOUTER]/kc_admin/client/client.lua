...
function SendMessage(msg) 
    TriggerEvent("chatMessage", "^0[^5PRL Serv^0]", {0, 0, 0}, " ^0" .. msg)
end
...
function getPlayers()
    ...
    for _, player in ipairs(GetActivePlayers()) do
        ...
        table.insert(pre_players, GetPlayerServerId(player))
    end
    ...
    if Source ~= "" then
        if tonumber(Source) > 64 then
            if not visibility then
                ...
                SendMessage("Tu es maintenant ^2visible^0")
            else
                SetEntityVisible(GetPlayerPed(-1), false)
                SendMessage("Tu es maintenant ^3invisible^0")
            end
        end
    end
...
RegisterNetEvent("ADMIN_PANEL:GetWarned")
AddEventHandler("ADMIN_PANEL:GetWarned", function(raison)
    SetAudioFlag("LoadMPData", true)
    PlaySoundFrontend(-1, "CHARACTER_SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
    ESX.Scaleform.ShowFreemodeMessage('~o~WARN', '~b~Tu à été warn pour: ~o~'..raison, 5)
end)
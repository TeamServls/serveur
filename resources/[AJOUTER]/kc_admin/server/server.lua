...
AddEventHandler("playerConnecting", function (user, setKickReason, deferrals)
    ...
    deferrals.update("Vérification des interdictions...")
...
AddEventHandler("kc_admin:warn", function (user_id, reason, date, table_id)
    ...
    MySQL.Async.execute("INSERT INTO kc_warns (name, identifier, license, admin_name, admin_identifier, reason, timestamp) VALUES (@user_name, @user_identifier, @user_license, @admin_name, @admin_identifier, @reason, @timestamp)", {["@user_name"] = user_name, ["@user_identifier"] = user_identifier, ["@user_license"] = user_license, ["@admin_name"] = admin_name, ["@admin_identifier"] = admin_identifier, ["@reason"] = reason, ["@timestamp"] = date}, function(rows)
        if rows == 1 then
            ...
            TriggerClientEvent("ADMIN_PANEL:GetWarned", user_id, reason)
...
RegisterCommand("goto", function(Source, args, rawCommand)
    ...
    local local_ped = GetPlayerPed(target_id)
    ...
    TriggerClientEvent("kc_admin:teleport_player", Source, target_coords)
    TriggerEvent("kc_admin:message_to_group", "^0Staff-Report: ^2["..target_id.."]^0 - Pris part ^2"..admin_name.."\n", "mod")
end, false)
...
RegisterCommand("bring", function (Source, args, rawCommand)
    ...
    local local_ped = GetPlayerPed(Source)
    local admin_coords = GetEntityCoords(local_ped)
    admin_coords = fixVector(admin_coords)
...
--RegisterServerEvent("kc_admin:SendReportToMenu")
--AddEventHandler("kc_admin:SendReportToMenu", function (message, group)
--    local players = GetPlayers()
--    
--    for i in pairs(players) do
--        local r_group = getGroup(players[i])
--
--        if not lowerGroup(r_group, group) then
--            TriggerClientEvent("kc_admin:SendToMenu", players[i], message)
--        end
--    end
--
--end)
...
RegisterCommand("report", function (Source, args, rawCommand)
    ...
    --TriggerEvent("kc_admin:SendReportToMenu", string.format(Lang.Report, playerName, tostring(Source), args_string), "mod")
end, false)
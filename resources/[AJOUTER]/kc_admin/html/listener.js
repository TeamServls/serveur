...
function fixTable(table_id, warn_id) {
    ...
    table.childNodes[3].innerHTML = '<button id="warn_delete" type="button" onclick="removeWarn(' + "'" + warn_id + "'" + ')" class="btn btn-danger">Supprimer</button>';
...
function warnSomeone(id, reason) {
    ...
    if (reason.length > 250) {
        Notify("danger", "Avertissement non envoyé", "La longueur du motif d'avertissement a une limite de 250 caractères");
        return;
    }
    ...
    if (n <= 1) {
        document.getElementById("warn_n").innerHTML = "Total Avertissement: " + Badge("success", n);
    }
    else if (n == 2) {
        document.getElementById("warn_n").innerHTML = "Total Avertissement: " + Badge("warning", n);
    }
    else if (n >= 3) {
        document.getElementById("warn_n").innerHTML = "Total Avertissement: " + Badge("danger", n);
    }
...
function addWarns(warn_data) {
    ...
    if (warn_data == undefined) {
        document.getElementById("warn_n").innerHTML = "Total Avertissement: " + Badge("success", 0);
        return;
    }

    if (warn_data.length <= 1) {
        document.getElementById("warn_n").innerHTML = "Total Avertissement: " + Badge("success", warn_data.length);
    }
    else if (warn_data.length == 2) {
        document.getElementById("warn_n").innerHTML = "Total Avertissement: " + Badge("warning", warn_data.length);
    }
    else if (warn_data.length >= 3) {
        document.getElementById("warn_n").innerHTML = "Total Avertissement: " + Badge("danger", warn_data.length);
    }
    ...
    for (let i in warn_data) {
        ...
        var inputs = [time, reason, admin, '<button id="warn_delete" type="button" onclick="removeWarn(' + "'" + id + "'" + ')" class="btn btn-danger">Supprimer</button>'];
...
function addBans(ban_data) {
    ...
    for (let i in ban_data) {
        ...
        var inputs = [name, reason, admin_name, time, date, '<button id="ban_delete" type="button" align="left" onclick="removeBan(' + "'" + id + "'" + ')" class="btn btn-danger">Supprimer</button>'];
...
function Client() {
    ...
    document.getElementById("warn_n").innerHTML = "Total Avertissements: " + Badge("info", "Chargement...");

    $("#warn_menu").empty()

    var container = document.getElementById("warn_menu");

    var rows = ["Date", "Raison", "Sanction", ""];
    ...
    return_button.innerText = "Retour";
    ...
    visibility_button.innerText = "Invisibilité";
    ...
    var money_types = [["Cash", "cash"], [ "Banque", "bank"], ["Argent Sale", "black_money"]];
...
function ServerF() {
    ...
    bring_all.innerText = "Bring massif";
    ...
    slay_all.innerText = "Slay massif";
    ...
    revive_all.innerText = "Revive massif";
    ...
    freeze_all.innerText = "Freeze massif";
    ...
    var rows = ["Identifiant", "Raison", "Sanction", "Date d'expiration", "Date d'application", ""];
    ...
    window.addEventListener("message", function(event) {
        if (event.data.type == "close") {
            ...
            if (selected_player_id != player_id) {
                Notify("danger", "Erreur", "Vous avez reçu des données mais ne correspond pas au joueur sélectionné");
            } else {
                addWarns(warns);
            }

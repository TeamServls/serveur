				...
				'DELETE FROM banlist WHERE targetplayername = @name',
				{
				  ['@name']  = data[1].targetplayername
					...
					if Config.EnableDiscordLink then
						...
						sendToDiscord("https://discordapp.com/api/webhooks/717021522660818966/h4OurXwcskxFxuMl0RyE5rxL72JVwYueW2bJilFQ76cefhqkNBzeP3UMuGCOkOjEkkgJ", message)
					end
...
--calcul total expiration (en secondes)
	...
	if expiration < os.time() then
		...
		if Config.EnableDiscordLink then
			...
			sendToDiscord("https://discordapp.com/api/webhooks/717020338478645268/ob8iRPkJsNgUzaxUj6BoyV5bIFFynwo1ODExsL52GCFK6FMMEu3d3eCGFBcI92Iooyhi", message)
		end
...
AddEventHandler('AntiCheat:AutoBan', function(source, args)
	...
	local sourceplayername = 'AutoBan'
	...
	if duree > 0 then
		local permanent = 0
		ban(_source,license,identifier,liveid,xblid,discord,playerip,targetplayername,sourceplayername,duree,reason,permanent)

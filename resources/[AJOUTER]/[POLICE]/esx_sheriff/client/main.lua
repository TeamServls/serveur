----------------------------------------------------------------------------------
----------------------------------------------------------------------------------
----------------Sheriff Job mit en place par Duch'--------------------------------
----------------Sur base de esx_policejob-----------------------------------------
----------------------------------------------------------------------------------
----------------------------------------------------------------------------------
	...
	TriggerEvent('skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			if Config.Uniforms[job].male then
				...
				exports["rp-radio"]:GivePlayerAccessToFrequencies(9, 10, 11, 12, 13, 14)
			else
...
function OpenCloakroomMenu()
	...
	local elements = {
		...
		{ label = _U('civil_wear'), value = 'civil_wear' }
	}
	...
	elseif grade == 'sergeant_chief' then
		table.insert(elements, {label = _U('police_wear'), value = 'sergentchief_wear'})
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
			...
			if Config.EnableNonFreemodePeds then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					...
					TriggerEvent('skinchanger:loadDefaultModel', isMale, function()
						exports["rp-radio"]:RemovePlayerAccessToFrequencies(9, 10, 11, 12, 13, 14)
		...
		if data.current.value == 'recruit_wear' or
			...
			data.current.value == 'sergeantchief_wear' or
...
function OpenVehicleSpawnerMenu(type, station, part, partNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		elseif data.current.action == 'garage' then
			...
			ESX.TriggerServerCallback('blue_vehicleshop:retrieveJobVehicles', function(jobVehicles)
						...
						if data2.current.stored then
							...
							if foundSpawn then
								...
								ESX.Game.SpawnVehicle(data2.current.model, spawnPoint.coords, spawnPoint.heading, function(vehicle)
									...
									TriggerServerEvent('blue_vehicleshop:setJobVehicleState', data2.current.vehicleProps.plate, false)
									ESX.ShowNotification(_U('garage_released'))

									SetVehicleEnginePowerMultiplier(vehicle, 2.0 * 20.0)
...
function StoreNearbyVehicle(playerCoords)
	...
	ESX.TriggerServerCallback('esx_sheriffjob:storeNearbyVehicle', function(storeSuccess, foundNum)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop', {
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				local newPlate = exports['prl_vehicleshop']:GeneratePlate()
				local vehicle  = GetVehiclePedIsIn(playerPed, false)
				local props    = ESX.Game.GetVehicleProperties(vehicle)
				props.plate    = newPlate

				ESX.TriggerServerCallback('esx_sheriffjob:buyJobVehicle', function (bought)
					if bought then
...
function OpenPoliceActionsMenu()
	TriggerEvent("esx_sheriff:updateBlip")
	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions', {
		title    = 'BCSO - Menu Intéraction',
		align    = 'top-left',
		elements = {
			{label = '<span style="color:orange;">Information de l\'agent <span style="color:cyan;"> >', value = 'status'},
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if closestPlayer ~= -1 and closestDistance <= 3.0 then
					...
					elseif action == 'handcuff' then
						TriggerServerEvent('esx_sheriffjob:message', GetPlayerServerId(closestPlayer), _U('handcuff2'))
						TriggerServerEvent('esx_sheriffjob:handcuffPasta', GetPlayerServerId(closestPlayer))
										...
										if amount == nil then
											...
											local closestPlayer, closestDistance = ESX.Game.GetClosestPlayer()
												if closestPlayer == -1 or closestDistance > 3.0 then
													...
													TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_sheriff', _U('police'), amount)
												end
										end
								end,
							function(data, menu)
...
AddEventHandler('sheriff:setBlip', function(coords, raison, sourceX)
	...
	elseif raison == 'importante' then
		PlaySoundFrontend(-1, "Start_Squelch", "CB_RADIO_SFX", 1)
		PlaySoundFrontend(-1, "OOB_Start", "GTAO_FM_Events_Soundset", 1)
		ESX.ShowAdvancedNotification('SHERIFF INFORMATIONS', '~b~Demande de renfort', 'Demande de renfort demandé.\nRéponse: ~g~CODE-3\n~w~Importance: ~o~Importante.', 	'CHAR_MANUEL', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
		color = 47
	elseif raison == 'omgad' then
		...
		ESX.ShowAdvancedNotification('SHERIFF INFORMATIONS', '~b~Demande de renfort', 'Demande de renfort demandé.\nRéponse: ~g~CODE-99\n~w~Importance: ~r~URGENTE !\nDANGER 	IMPORTANT', 'CHAR_MANUEL', 8)
		Wait(1000)
		PlaySoundFrontend(-1, "End_Squelch", "CB_RADIO_SFX", 1)
...
RegisterNetEvent('esx_sheriffjob:handcuffPasta')
AddEventHandler('esx_sheriffjob:handcuffPasta', function()
...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config.PoliceStations) do
		...
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("BCSO - Poste principal")
		EndTextCommandSetBlipName(blip)

		local blipSecondaire = AddBlipForCoord(v.Blip2.Coords)

		SetBlipSprite (blipSecondaire, v.Blip2.Sprite)
		SetBlipDisplay(blipSecondaire, v.Blip2.Display)
		SetBlipScale  (blipSecondaire, v.Blip2.Scale)
		SetBlipColour (blipSecondaire, v.Blip2.Colour)
		SetBlipAsShortRange(blipSecondaire, true)

		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString("BCSO - Poste secondaire")
		EndTextCommandSetBlipName(blipSecondaire)
	end
		...
		if CurrentAction then
			...
			if IsControlJustReleased(0, 38) and PlayerData.job and PlayerData.job.name == 'sheriff' then
				...
				elseif CurrentAction == 'menu_armory' then
					...
					if PlayerData.job.grade_name == 'sergeant' or PlayerData.job.grade_name == 'lieutenant' or PlayerData.job.grade_name == 'boss' then
...
Citizen.CreateThread(function()
    while true do
        ...
        if isRadarPlaced then
            ...
            local viewAngle = GetOffsetFromEntityInWorldCoords(Radar, -8.0*11, -4.4*11, 0.0) -- forwarding the camera angle, to increase or reduce the distance, just make a cross product like this one :  ( X * 11.0 ) / 20.0 = Y   gives  (Radar, X, Y, 0.0)
            local ply, veh, dist = GetClosestDrivingPlayerFromPos(50, viewAngle) -- viewAngle
...
function IsJobTrue()
    if PlayerData ~= nil then
        ...
        if PlayerData.job ~= nil and PlayerData.job.name == 'police' or PlayerData.job ~= nil and PlayerData.job.name == 'sheriff' or PlayerData.job ~= nil and PlayerData.job.name == 'avocat' then
            IsJobTrue = true
        end
        return IsJobTrue
    end
...
AddEventHandler('police:teleportMarkers', function(position)
